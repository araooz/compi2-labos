...
Token* Scanner::nextToken() {
    ...
    if(isalpha(c)) {
        ...
        if (lexema == "sqrt") {
            token = new Token(Token::SQRT, lexema, 0, lexema.length());
        }
        else if (lexema == "log") {
            token = new Token(Token::LOG, lexema, 0, lexema.length());
        }
        else if (lexema == "pow") {
            token = new Token(Token::POW2, lexema, 0, lexema.length());
        }
        else {
            token = new Token(Token::ID, lexema, 0, lexema.length());
        } 
    }
        ...
        switch (c) {
            ...
            case '*': 
             if (current + 1 < input.length() && input[current + 1] == '*') {
                token = new Token(Token::POW, "**",0,2); // Token de potencia
                current++; // Avanzar un carácter adicional para el segundo '*'
            }
            else {
                token = new Token(Token::MUL, c); // Token de multiplicación
            }
            break;
